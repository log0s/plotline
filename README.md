...
**Cross-source comparison is imperfect.** Each imagery source has its own footprint, resolution, band layout, and acquisition geometry. A 1m NAIP tile and a 30m Landsat scene cover very different areas at very different detail, and successive NAIP acquisitions don't always cover the exact same extent. Every source is served onto the same Web Mercator tile grid, so the map lines up — but that is a per-tile reprojection done by the tile server, not a normalization step: nothing resamples sources to a common ground resolution, and each source carries a fixed display stretch rather than a per-scene one, so brightness and color shift between eras. Comparing fine-grained change over a specific parcel across sources can still be difficult.
