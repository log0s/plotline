...
| # | Status | Where it stands |
|---|---|---|
| NORM-1 | **New — built, run locally, prediction scored. Committed, NOT deployed, and the production backfill has NOT run (2026-08-28).** | **`scenes` and `parcel_scenes` exist and are filled from `imagery_snapshots` locally.** Migration 0015 (`822faca`) is pure DDL; ORM models and the mirrored SQLite test DDL are `0fc0f64`; `scripts/backfill_scenes.py` is `b3f8e94`; tests `382329e`. Locally: 2,945 snapshot rows → **1,262 scenes** (1,174 from distinct `(stac_collection, stac_item_id)`, 88 synthesized from mosaic URLs) and **2,945 parcel_scenes**, every predicted quantity confirmed with no deviations (`ea98325` predicted, `d13026e` scored). A second `--execute` run inserted nothing and reported no drift. **Nothing is deployed and no production row has been written**: 0015 has not run against production and the prod backfill is the next action, expected to produce `scenes = 6,156 + D` (D = distinct unmatched mosaic URLs, ≤ 540) and `parcel_scenes` equal to the `imagery_snapshots` count read in the same transaction — *not* 12,884, which is a 2026-08-28 reading of a table still taking traffic. **No read path moved and `reconcile_source_snapshots` is unmodified**; steps 2 and 3 own those. |
...
1. **Imagery normalization** (`docs/adr/0001-imagery-normalization.md`) —
   ...
   **2026-08-28: step 1 is built, run locally and scored (NORM-1 above); the
   ADR is Accepted. Steps 2–4 not started, and step 1 is not deployed — the
   production backfill has not run.** Next action is deploying 0015 and
   running `scripts/backfill_scenes.py --execute` against production, both
   Ryan's.
