...
| X3 | **MEDIUM — Open. Nothing in CI can fail on a migration that does not persist.** | `.github/workflows/deploy.yml` contains **no alembic step**, and `backend/tests/conftest.py` builds its schema as hand-written DDL against an in-memory SQLite engine (`conftest.py:33-223`). The migration directory is therefore never executed anywhere in the pipeline. The 488-test suite that ships with `0011` passes because `_create_test_tables()` creates its tables, not because the migration works. This is the same hand-written-DDL fact M7 already records as an ORM/schema-drift risk; what M7 does not say is that it is also a **migration-execution** risk, and X1 is what that costs. Any fix for X1 whose test runs on SQLite or on `Base.metadata.create_all` is not a test of X1. |
