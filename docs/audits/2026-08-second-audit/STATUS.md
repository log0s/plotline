...
- **Adams County property returns empty, every time.** The single Adams parcel
  ...
  **CLOSED as code, 2026-08-27 (`eee8a9e`), deployed `fbdc2f7`
  2026-08-27T23:17Z — pending fleet sweep.** `DEPLOY-WATCH.md`: migration
  0014's `coverage` CHECK confirmed live (`covered`/`not_covered`/
  `no_adapter`, nullable), zero pre-existing rows populated. The Adams
  parcel's most recent property task predates this deploy and still reads
  `complete:0` with `coverage=NULL`, as expected — the gate itself has not
  fired on any live request yet; that's the next sweep. `CountyAdapter.covers(city)` defaults to True
