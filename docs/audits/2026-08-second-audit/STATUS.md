...
M4's remedy is a heal, and no heal has run.* **M7-5** is a row split out of M7, not a thirteenth Medium
finding.
...
| Y7 | **Open — found while scoring M3 heal 3, 2026-08-27. Not fixed.** | **The ledger cannot mark an absence permanent, so a permanent absence is retried forever.** `absent`/`api_no_data` is written both by a request that was wrong and has since been fixed and by a tract the Census API has genuinely never held data for. `--include-absent-api` cannot tell them apart, so it selects both. Measured: after the decennial-2000 sweep healed 63 of 139 parcels, **76 re-recorded `absent`/`api_no_data` for `census_decennial`/`2000`, and all 76 are permanent** — 60 carry a real tract suffix that did not exist in 2000 (the trim deliberately cannot reach them; trimming a real suffix would substitute a coarser geography and label it with the parcel's tract), and 16 end `00` but 204 under the four-character form too, verified twice (`../2026-08-census-decennial/REPORT.md` §1.5, live 2026-08-26; re-recorded from production 2026-08-27). **The next `--from-ledger --sources census_decennial --include-absent-api` run selects all 76 again** and spends ~9 Census API calls plus ~4.5 s of inter-year sleep per parcel to write the identical row. **This is Y3's sibling, not Y3.** Y3 was a group current code *no longer attempts* (`1990`), closed by `imagery.attempted_group_keys` + `ledger.is_stale`. This is a group current code *does* attempt, correctly, whose answer will not change — `is_stale` cannot reach it and should not. Fixing it needs a vocabulary the ledger does not have (a reason that means "absent, and re-asking will not help", e.g. splitting `api_no_data` from an `api_no_data_permanent`, or an operator-set suppression keyed on the triple), which is a design decision rather than a patch and is deliberately not made here. **Scale today is small and bounded** — 76 parcels, one group each, only behind an explicit flag that no self-running path passes — which is why it is recorded rather than fixed; it becomes a real cost when the fleet grows or when a second dataset acquires a permanent-absence population. Evidence: `../2026-08-m3/HEAL-3-decennial-2000.md` §5.4. |
