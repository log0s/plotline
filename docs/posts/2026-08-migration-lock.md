...
facts_to_verify:
  - "Test count — RESOLVED, and the record is wrong. GATE-STOP.md §6 and the X3 row both say 'the 488-test suite that ships with 0011'. Collected at ce307e35 (the SHA that shipped 0011 to production) the suite is 522, and 525 at edc13db; REPORT.md §6 says 522, 'up from 488 at fa3ea89', so 488 is the pre-ledger count and §6 and X3 carry it forward by mistake. The post says 522 and 525. STATUS.md's X3 row needs a correction annotation this batch was not scoped to make."
  - "env.py line ranges — RESOLVED against the file at edc13db. GATE-STOP.md §12 is right (_verify_at_head at :56-95, the transaction block at :141-191); the X1 row (:52-86, :141-194) and the commit message (:36-86, :141-194) are both slightly off. The post cites env.py:96-109 for the pre-fix block at ce307e35, which STATUS.md's X1 row and the file agree on."
  - "Twenty-two days, not 23. dd99cee is authored 2026-08-03T17:56-06:00 = 2026-08-04T00:56Z; the gate's capture clock is 2026-08-26T00:59:39Z. STATUS.md's M10 re-mark says 23 days, which is the difference between the two local calendar dates. The post is UTC throughout and says twenty-two."
  - "The 00:52Z boots never overlapped. 825d69b7e46618 logs 'Migrations complete.' at 00:52:28Z; 48e0de9a713918 logs 'Running database migrations...' at 00:52:40Z. GATE-STOP.md §10 anomaly 1 reads the second run as the lock failing to serialize; on these timestamps there was nothing to serialize, and the second boot re-ran the upgrade because the version bump had been rolled back. The post says the latter. Worth reconciling with §10 before publishing."
  - "No log line proves either boot ever waited on the lock. At 01:29Z the first machine's head check lands at 01:29:41Z and the second pulls its image at 01:29:51Z, so that pair did not demonstrably overlap either. What is observed is that the second boot found the database at head and ran no upgrade — the outcome M10 wanted. GATE-STOP.md's addendum and the X1 row both describe it as the lock serializing two boots; the post claims the outcome, not an observed wait."
  - "That no production timeline request was lost. Rests on zero timeline_requests rows created between 00:52Z and 01:36Z, plus 0 queued and 0 processing. GATE-STOP.md §9 bounds this to *created* requests and notes the retained log buffer is not proof on its own."
  - "The gate quote. SWEEP-PROMPT-1.md Phase 1 line 2 reads in full: 'alembic_version on prod reads 0011. timeline_task_years exists, has the expected columns, constraints, and indexes, and holds zero rows. Otherwise stop — a non-empty ledger before the sweep means something already ran.' The post elides the columns/constraints/indexes clause with an ellipsis. GATE-STOP.md §1's gate table condenses the same line to 'alembic_version = 0011; timeline_task_years exists, empty' and drops the rationale clause entirely — the prompt is the source, the table is the paraphrase."
  - "SQLAlchemy version on production. GATE-STOP.md §3 says 2.0.49 and §15 says the local reproduction ran 2.0.50. The post names neither; if a version is added, use §15."
---
